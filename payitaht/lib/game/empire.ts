...
export const ISLANDS = [
  { id: 'sahil', name: 'Sahil Adası', x: 4, y: 5, specialty: 'Mermer' },
  { id: 'zeytin', name: 'Zeytin Adası', x: 8, y: 3, specialty: 'Üzüm' },
  { id: 'akcam', name: 'Akçam Adası', x: 2, y: 2, specialty: 'Kristal' },
  { id: 'kizil', name: 'Kızılburun', x: 10, y: 7, specialty: 'Kükürt' },
  { id: 'akdeniz', name: 'Akdeniz Adası', x: 6, y: 9, specialty: 'Mermer' },
  { id: 'yalcin', name: 'Yalçın Ada', x: 12, y: 2, specialty: 'Kristal' },
  { id: 'baglik', name: 'Bağlık Ada', x: 1, y: 8, specialty: 'Üzüm' },
  { id: 'atessiz', name: 'Ateşli Ada', x: 11, y: 11, specialty: 'Kükürt' },
] as const
...
export type Shipment = {
  id: string; from: string; to: string; resource: Resource
...
export function parseEmpire(raw: string): Empire {
  ...
  const cities: CityRecord[] = obj.cities.map(city => {
    ...
    return { ...city, game: parseSave(JSON.stringify(city.game)) }
  })
  ...
  const shipments: Shipment[] = obj.shipments.map(s => {
    if (!s || typeof s.id !== 'string' || s.id.length > 80 || !ids.has(s.from) || !ids.has(s.to) ||
        s.from === s.to || !['gold','wood','stone','knowledge'].includes(s.resource) ||
...
export function advanceEmpire(source: Empire, now: number): Empire {
  ...
  for (const shipment of empire.shipments) {
    ...
    const free = Math.max(0, Math.floor(capacity(destination.game) - destination.game.resources[shipment.resource]))
    const delivered = Math.min(shipment.amount, free)
    destination.game.resources[shipment.resource] += delivered
...
export function shipResources(source: Empire, to: string, resource: Resource, amount: number, now: number):
  { empire: Empire; error?: string } {
  const empire = advanceEmpire(source, now)
  ...
  if (!['gold', 'wood', 'stone', 'knowledge'].includes(resource) ||
  ...
  if (amount > from.game.resources[resource]) return { empire, error: 'Bu kadar kaynak bulunmuyor.' }
