...
import { ArrowUp, Hammer, Clock3, LockKeyhole, Check, BookOpen, ChevronRight, TreePine, Warehouse, Ruler, Users, UserRound, Minus, Plus as PlusIcon } from 'lucide-react'
import { Button } from '@/components/ui/button'
import { CostDisplay, JobProgress } from './game-widgets'
import { BUILDINGS, BUILDING_IDS, RESEARCH, RESEARCH_IDS, WORKER_IDS, WORKERS_PER_LEVEL, cost, duration, buildReason, researchReason, idleWorkers, population, workerCapacity, type BuildingId, type ResearchId, type WorkerId, type Game } from '@/lib/game/engine'
...
export function BuildingDetails({ game, id, onBuild }: { game: Game; id: BuildingId; onBuild: (id: BuildingId) => void }) {
  ...
  const active = game.construction?.id === id ? game.construction : null
  return <div className="building-details"><div className="building-preview"><div className="preview-halo" /><img src={`/images/game/${id}.webp`} alt={`${b.name} mimari görünümü`} width={360} height={360} /><span>{level ? `SEVİYE ${level}` : 'YENİ YAPI'}</span></div><span className="eyebrow">{b.category}</span><p>{b.description}</p><div className="building-upgrade"><span>{level ? `Seviye ${level}` : 'Boş arsa'}</span><ArrowUp className="size-4" /><strong>{level >= 5 ? 'En yüksek seviye' : `Seviye ${level + 1}`}</strong></div>{active ? <JobProgress job={active} now={game.updatedAt} /> : level < 5 && <><div className="upgrade-cost"><span>Gerekli kaynaklar</span><CostDisplay value={cost(game, id)} /></div><div className="duration-row"><Clock3 className="size-4" /> {duration(game, id)} saniye <span>Prototip süresi</span></div></>}{reason && !active && <p className="requirement"><LockKeyhole className="size-4" />{reason}</p>}<Button size="lg" className="w-full" disabled={!!reason} onClick={() => onBuild(id)}><Hammer data-icon="inline-start" />{active ? 'İnşaat devam ediyor' : level >= 5 ? 'Tamamen geliştirildi' : level ? 'Binayı yükselt' : 'İnşaata başla'}</Button></div>
}
export function BuildingList({ game, onSelect }: { game: Game; onSelect: (id: BuildingId) => void }) {
  return <div className="building-list">{BUILDING_IDS.map(id => <button key={id} className="building-list-item" onClick={() => onSelect(id)}><img src={`/images/game/${id}.webp`} alt="" width={88} height={88} /><span><span className="eyebrow">{BUILDINGS[id].category}</span><strong>{BUILDINGS[id].name}</strong><span>{game.buildings[id] ? `Seviye ${game.buildings[id]}${game.buildings[id] >= 5 ? ' · Tamamlandı' : ' · Geliştirilebilir'}` : 'Boş arsa · Yeni yapı'}</span></span><ChevronRight className="size-4" /></button>)}</div>
}
  ...
  return <div className="building-list">
    ...
    {candidates.map(id => {
      ...
      return <button key={id} className="building-list-item" disabled={!!reason} onClick={() => onBuild(id, plot)}>
        <img src={`/images/game/${id}.webp`} alt="" width={88} height={88} />
        <span>
...
export function PeoplePanel({ game, onAssign }: { game: Game; onAssign: (id: WorkerId, value: number) => void }) {
  ...
  return <div className="people-panel">
    ...
    <p className="fine-print">Her yapı seviyesi {WORKERS_PER_LEVEL} işçi alır. Boşta kalan halk üretim yapmaz; akçe ise halkın kendisinden gelir ve işçi istemez.</p>
  </div>
}
