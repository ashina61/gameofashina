...
import { Hammer, Download, RotateCcw, HardDrive, WifiOff, Skull, X, Check, Bell } from 'lucide-react'
...
import { DefenseSummary, ExchangePanel, FuturePanel, GuildPanel, PiracyPanel, TemplePanel, ThreatBanner, UpgradePanel } from './ikariam-panels'
...
type Panel = 'build' | 'research' | 'journal' | 'settings' | 'economy' | 'objectives' | 'people' | 'cities' | 'army' | 'diplomacy' | 'island' | 'reports' | 'advisor-city' | 'profile' | 'changelog' | 'map' | null
export default function GameShell() {
  ...
  const navActive: IkaNavKey | null = panel === 'map' ? 'map' : panel === 'objectives' ? 'objectives'
    : view === 'island' ? 'island' : !panel && !selected && plot === null && !npc ? 'city' : null
  const titles: Record<Exclude<Panel, null>, string> = { build: 'Şehrini büyüt', research: 'Âlim · Araştırma', journal: 'Şehir günlüğü', settings: 'Oyun ayarları', economy: 'Hazine ve üretim', objectives: 'Bir şehrin doğuşu', people: 'Şehrin halkı', cities: 'Şehirlerin', army: 'Ordu ve donanma', diplomacy: 'Elçi · Diplomasi', island: 'Ada ve maden', reports: 'Serasker · Ordu danışmanı', 'advisor-city': 'Vezir · Şehir danışmanı', profile: 'Hükümdar profili', changelog: 'Sürüm notları', map: 'Dünya haritası' }
  return <main className={cn('game-shell', view === 'island' && 'game-island')}>
    ...
    {game && selected && <BuildingPage game={game} empire={empire} id={selected} onClose={() => setSelected(null)}
      ...
      onNav={p => { setSelected(null); if (p === 'island') { setView('island'); setPanel('island') } else openPanel(p) }}>{selected === 'divan' && empire && <CityAdmin empire={empire} game={game} now={game.updatedAt} onCommand={act} run={runOp} />}{selected === 'kahvehane' && <TavernPanel game={game} onCommand={act} />}{selected === 'medrese' && <ExperimentPanel game={game} onCommand={act} />}{selected === 'tekke' && <GuildPanel game={game} now={game.updatedAt} onCommand={act} />}{selected === 'mabet' && <GodsPanel game={game} now={game.updatedAt} onCommand={act} />}{selected === 'cami' && <TemplePanel game={game} now={game.updatedAt} onCommand={act} />}{selected === 'tophane' && <UpgradePanel game={game} onCommand={act} />}{selected === 'kara_pazar' && <ExchangePanel game={game} onCommand={act} />}{selected === 'korsan_kalesi' && empire && <PiracyPanel empire={empire} now={game.updatedAt} onPiracy={(id, units) => { const e = piracy(id, units); if (e) toast.error(e); else toast.success('Filo denize açıldı.') }} />}</BuildingPage>}
