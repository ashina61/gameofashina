...
import { BuildingDetails, BuildingList, ResearchPanel, JournalPanel, PlotPicker, PeoplePanel } from './game-panels'
import { ResourceBar, QueueCard, ObjectiveCard, EconomyDetails } from './game-widgets'
import { useGame } from '@/hooks/use-game'
import { usePwa } from '@/hooks/use-pwa'
import { BUILDINGS, idleWorkers, population, type BuildingId, type Command } from '@/lib/game/engine'
import { cn } from '@/lib/utils'

type Panel = 'build' | 'research' | 'journal' | 'settings' | 'economy' | 'objectives' | 'people' | null
...
export default function GameShell() {
  ...
  function openPanel(value: Panel) { setSelected(null); setPlot(null); setPanel(value); setConfirmReset(false) }
  ...
  function openPlot(index: number) { setPanel(null); setSelected(null); setPlot(index) }
    ...
    toast.success(action.type === 'build' ? 'Ustalar iş başında. İnşaat başladı!' : action.type === 'research' ? 'Yeni bir keşfe doğru. Araştırma başladı!' : 'Ödül hazinene eklendi.')
  }
  ...
  const titles: Record<Exclude<Panel, null>, string> = { build: 'Şehrini büyüt', research: 'İlim ve keşif', journal: 'Şehir günlüğü', settings: 'Oyun ayarları', economy: 'Hazine ve üretim', objectives: 'Bir şehrin doğuşu', people: 'Şehrin halkı' }
