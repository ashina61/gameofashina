...
import { MoonStar, Settings2, Bell, Crown, House, Hammer, BookOpen, ShieldCheck, Download, RotateCcw, HardDrive, WifiOff, Users, Sprout, Swords, Skull } from 'lucide-react'
...
export default function GameShell() {
  ...
  return <main className="game-shell">
    ...
    {game ? <><ResourceBar game={game} onSelect={() => openPanel('economy')} /><AdvisorBar game={game} active={panel} onSelect={openPanel} /><div className="game-body"><div className="city-column"><CityScene game={game} placing={buildMode || plot !== null} onBuilding={openBuilding} onPlot={openPlot} onRoad={cell => act({ type: 'road', cell })} onExitBuild={() => setBuildMode(false)} onOpenList={() => openPanel('build')} /></div></div>
      {/*
      ...
      <button className="hud-ticker" onClick={() => openPanel('journal')} aria-label="Şehir günlüğü">
        ...
        <span className="hud-ticker-text">{game.log[0]?.text ?? 'Sahilhisar sessiz.'}</span>
      </button></> : <div className="game-loading"><img src={buildingImage('divan')} alt="" width={150} height={150} /><h1>Şehrin uyanıyor…</h1><p>Sahilhisar kapılarını açıyor.</p></div>}
