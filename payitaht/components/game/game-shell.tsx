...
export default function GameShell() {
  ...
  return <main className={cn('game-shell', view === 'island' && 'game-island')}>
    ...
    {game && selected && <BuildingPage game={game} empire={empire} id={selected} onClose={() => setSelected(null)}
      ...
      onNav={p => { setSelected(null); if (p === 'island') { setView('island'); setPanel('island') } else openPanel(p) }}>{selected === 'divan' && empire && <CityAdmin empire={empire} game={game} now={game.updatedAt} onCommand={act} run={runOp} />}{selected === 'kahvehane' && <TavernPanel game={game} onCommand={act} />}{selected === 'medrese' && <ExperimentPanel game={game} onCommand={act} />}{selected === 'tekke' && <GuildPanel game={game} now={game.updatedAt} onCommand={act} />}{selected === 'cami' && <TemplePanel game={game} now={game.updatedAt} onCommand={act} />}{selected === 'tophane' && <UpgradePanel game={game} onCommand={act} />}{selected === 'kara_pazar' && <ExchangePanel game={game} onCommand={act} />}{selected === 'korsan_kalesi' && empire && <PiracyPanel empire={empire} now={game.updatedAt} onPiracy={(id, units) => { const e = piracy(id, units); if (e) toast.error(e); else toast.success('Filo denize açıldı.') }} />}</BuildingPage>}
