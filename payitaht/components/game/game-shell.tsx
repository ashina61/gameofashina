...
type Panel = 'build' | 'research' | 'journal' | 'settings' | 'economy' | 'objectives' | 'people' | 'cities' | 'army' | 'diplomacy' | 'island' | null
/*
 * ALT MENU DORT OGE.
 *
...
export default function GameShell() {
  ...
  function cancelMove() { setMoving(null); setMovePlot(null) }
    ...
    if (action.type === 'workers' || action.type === 'miners' || action.type === 'road' || action.type === 'flip') return
  ...
  const titles: Record<Exclude<Panel, null>, string> = { build: 'Şehrini büyüt', research: 'İlim ve keşif', journal: 'Şehir günlüğü', settings: 'Oyun ayarları', economy: 'Hazine ve üretim', objectives: 'Bir şehrin doğuşu', people: 'Şehrin halkı', cities: 'Şehirlerin', army: 'Ordu ve donanma', diplomacy: 'Diplomasi', island: 'Ada ve maden' }
